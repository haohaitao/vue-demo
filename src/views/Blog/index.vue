  ...
  <article class="detail-article" v-loading="loading">
    <div class="art-header">
      ...
      <div class="header-tag">
        <el-tag v-for="item in tagData" :key="item.id" @click="jump(item)">{{item.name}}</el-tag>
      </div>
      <div class="tag-time">
    ...
    <div class="content-footer">
      <p>
        本文由
        <router-link to="/">{{blog.author === 1 ? '一只' : '博主'}}</router-link>创作，转载请注明
      </p>
      <p>最后编辑时间：{{(blog.modified ? blog.modified.split('T')['0'] :'1970-01-01') + ' ' + (blog.modified ? blog.modified.split('T')['1'] : '00:00:00')}}</p>
      <div id="vcomments"></div>
    </div>
    <el-button @click="drawerChange" type="primary" style="margin-left: 16px;">更多相同文章</el-button>
    <el-drawer title="相似文章" direction="ltr" :with-header="false" :visible.sync="drawerState">
